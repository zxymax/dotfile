...
本配置使用 `lazy-lock.json` 文件来锁定插件的具体版本，确保在多台机器上获得一致的插件体验。重要说明：

- **锁定文件状态**：当前 `lazy-lock.json` 在 `.gitignore` 中被忽略，这意味着每个人可以有自己的插件版本。
- **锁定插件版本**：运行 `:Lazy sync` 会更新 `lazy-lock.json` 文件，记录当前所有插件的确切版本。
- **团队协作建议**：如果你在团队环境中使用，可以考虑将 `lazy-lock.json` 从 `.gitignore` 中移除并提交到版本控制，以确保所有人使用相同的插件版本。
- **一致环境**：在CI/CD环境或需要一致配置的场景下，建议提交 `lazy-lock.json` 文件。
